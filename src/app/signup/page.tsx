...
import { Eye, EyeOff, CheckCircle, AlertCircle, User, Stethoscope, Shield } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const roleDescriptions = {
    ...
    ADMIN: 'System administration, user management, and hospital analytics (Official email required)'
  }
  ...
  const adminEmailDomains = ['gov.rw', 'hospital.rw', 'gov', 'hospital.com']
  ...
  const validateStep1 = () => {
    ...
    if (formData.role === 'ADMIN') {
      ...
      const isValidAdminEmail = adminEmailDomains.some(domain => 
        emailDomain?.includes(domain)
      )
      if (!isValidAdminEmail) {
        setError('Administrator must use an official email address (gov.rw, hospital.rw, etc.)')
        return false
      }
    }
  ...
  const renderRoleSelection = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Select your role in the healthcare system</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {Object.entries(roleDescriptions).map(([role, description]) => {
          ...
          return (
            <div
              key={role}
              onClick={() => setFormData(prev => ({ ...prev, role: role as UserRole }))}
      ...
      >
        Continue with {formData.role} role
      </button>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={handleInputChange}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder={formData.role === 'ADMIN' ? 'admin@hospital.rw' : 'john@hospital.com'}
        />
        {formData.role === 'ADMIN' && (
          <p className="text-sm text-orange-600 mt-1">
            Official email address required (gov.rw, hospital.rw, etc.)
          </p>
        )}
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={handleInputChange}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="Rwanda Military Hospital"
        />
      </div>
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">
            Join NeoTrack healthcare system
          </p>
        </div>
        ...
        <form className="bg-white shadow-lg rounded-lg p-8 space-y-6">
          ...
          {step === 3 && renderPasswordSetup()}
          {step === 4 && renderProfessionalInfo()}
